...
def get_logger(strategy_prefix: str = None) -> TrainingLogger:
    """Get or create global logger with optional strategy prefix for separate logs."""
    global _logger
    if _logger is None:
        if strategy_prefix:
            log_dir = os.path.join("logs", strategy_prefix)
        else:
            log_dir = "logs"
        _logger = TrainingLogger(log_dir=log_dir)
    return _logger


def reset_logger(strategy_prefix: str = None):
    """Reset logger for new session with optional strategy prefix."""
    global _logger
    if strategy_prefix:
        log_dir = os.path.join("logs", strategy_prefix)
    else:
